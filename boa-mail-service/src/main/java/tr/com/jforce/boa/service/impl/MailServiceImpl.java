...
import tr.com.jforce.boa.model.BaseMailRequest;
...
public class MailServiceImpl implements MailUtilService {
    ...
    public SimpleMailMessage templateForSimpleMessage() {
        ...
        simpleMailMessage.setSubject("Your account has been created");
	...
	@Override
	public void sendSimpleMail(BaseMailRequest sendMailRequest) throws MessagingException {
		 SimpleMailMessage simpleMailMessage = templateForSimpleMessage();
	        MimeMessage mimeMessage = mailSender.createMimeMessage();
	        try {
	            MimeMessageHelper mimeMessageHelper = new MimeMessageHelper(mimeMessage, true);
	            mimeMessageHelper.setText(sendMailRequest.getContent());
